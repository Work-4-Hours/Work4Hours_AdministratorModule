...
namespace Backend_CSharp_Work4Hours_AdministratorModule.Models.Entities
{
    public class Users
    {
        ...
        {
            string sql = "SELECT u.idusuario as id, SUBSTRING_INDEX(u.nombres, ' ', 1) as nombreUsuario, u.apellidos, u.correo, case when u.fotop is null then '' else u.fotop end as fotop, u.color, count(ur.id) as cantidadReportes, e.id as idEstado, e.nombre_estado FROM usuarios u INNER JOIN estados e on u.estado = e.id LEFT JOIN usuario_reportes ur on u.idusuario = ur.idusuario group by u.idusuario order by count(ur.id) desc;";
        ...
        public string searchUsersFilters(int option, string searchWord)
        {
            ...
            if (option == 1)
            {
                sql = $"select u.idusuario as id, SUBSTRING_INDEX(u.nombres, ' ', 1) as nombreUsuario, u.apellidos, u.correo, case when u.fotop is null then '' else u.fotop end as fotop, u.color, count(ur.id) as cantidadReportes, e.id as idEstado, e.nombre_estado from usuarios u inner join estados e on u.estado = e.id left join usuario_reportes ur ON u.idusuario = ur.idusuario where e.nombre_estado like '{searchWord}%' group by u.idusuario";
            }
            else if (option == 2)
            {
        ...
        public string suspensionUsers(List<ChangeState> array)
        {
            string sql = " ";
